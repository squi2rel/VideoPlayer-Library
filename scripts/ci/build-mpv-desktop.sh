...
setup_toolchain() {
  ...
  export CFLAGS="-O2 -pipe -fPIC -I$prefix/include ${CFLAGS:-}"
  export CXXFLAGS="-O2 -pipe -fPIC -I$prefix/include ${CXXFLAGS:-}"
...
prefer_static = true
c_args = ['-O2', '-pipe', '-fPIC', '-I$prefix/include', '-m32']
cpp_args = ['-O2', '-pipe', '-fPIC', '-I$prefix/include', '-m32']
...
default_library = 'static'
prefer_static = true
c_args = ['-O2', '-pipe', '-fPIC', '-I$prefix/include', '-mfpu=vfpv3-d16', '-mfloat-abi=hard']
cpp_args = ['-O2', '-pipe', '-fPIC', '-I$prefix/include', '-mfpu=vfpv3-d16', '-mfloat-abi=hard']
...
build_ffmpeg() {
  ...
  if [[ "$target_os" == "macos" ]]; then
    args+=(--enable-audiotoolbox --enable-videotoolbox)
  else
    args+=(
      --enable-alsa
      --enable-libjack
